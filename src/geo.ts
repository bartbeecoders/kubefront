// Offline geocoding for the dashboard world map.
//
// KubeFront often runs in air-gapped OT networks, so we do NOT call any online
// geocoding API. A connection's map position is resolved as:
//   1. explicit `latitude`/`longitude` on the entry (precise plant pin), else
//   2. the centroid of its `country` from the bundled table below.
// Coordinates are stored/returned as [longitude, latitude] — the order
// react-simple-maps' <Marker coordinates> expects.

import type { KubeconfigEntry } from "./types";

/** Approximate country centroids, [longitude, latitude]. Keyed by lowercased name. */
export const COUNTRY_CENTROIDS: Record<string, [number, number]> = {
  afghanistan: [67.71, 33.94],
  albania: [20.17, 41.15],
  algeria: [1.66, 28.03],
  angola: [17.87, -11.2],
  argentina: [-63.62, -38.42],
  armenia: [45.04, 40.07],
  australia: [133.78, -25.27],
  austria: [14.55, 47.52],
  azerbaijan: [47.58, 40.14],
  bahrain: [50.64, 26.07],
  bangladesh: [90.36, 23.68],
  belarus: [27.95, 53.71],
  belgium: [4.47, 50.5],
  bolivia: [-63.59, -16.29],
  "bosnia and herzegovina": [17.68, 43.92],
  brazil: [-51.93, -14.24],
  bulgaria: [25.49, 42.73],
  cambodia: [104.99, 12.57],
  cameroon: [12.35, 7.37],
  canada: [-106.35, 56.13],
  chile: [-71.54, -35.68],
  china: [104.2, 35.86],
  colombia: [-74.3, 4.57],
  "costa rica": [-83.75, 9.75],
  croatia: [15.2, 45.1],
  cuba: [-77.78, 21.52],
  cyprus: [33.43, 35.13],
  czechia: [15.47, 49.82],
  "czech republic": [15.47, 49.82],
  denmark: [9.5, 56.26],
  "dominican republic": [-70.16, 18.74],
  ecuador: [-78.18, -1.83],
  egypt: [30.8, 26.82],
  "el salvador": [-88.9, 13.79],
  estonia: [25.01, 58.6],
  ethiopia: [40.49, 9.15],
  finland: [25.75, 61.92],
  france: [2.21, 46.23],
  georgia: [43.36, 42.32],
  germany: [10.45, 51.17],
  ghana: [-1.02, 7.95],
  greece: [21.82, 39.07],
  guatemala: [-90.23, 15.78],
  honduras: [-86.24, 15.2],
  "hong kong": [114.11, 22.4],
  hungary: [19.5, 47.16],
  iceland: [-19.02, 64.96],
  india: [78.96, 20.59],
  indonesia: [113.92, -0.79],
  iran: [53.69, 32.43],
  iraq: [43.68, 33.22],
  ireland: [-8.24, 53.41],
  israel: [34.85, 31.05],
  italy: [12.57, 41.87],
  "ivory coast": [-5.55, 7.54],
  jamaica: [-77.3, 18.11],
  japan: [138.25, 36.2],
  jordan: [36.24, 30.59],
  kazakhstan: [66.92, 48.02],
  kenya: [37.91, -0.02],
  kuwait: [47.48, 29.31],
  latvia: [24.6, 56.88],
  lebanon: [35.86, 33.85],
  libya: [17.23, 26.34],
  lithuania: [23.88, 55.17],
  luxembourg: [6.13, 49.82],
  malaysia: [101.98, 4.21],
  malta: [14.38, 35.94],
  mexico: [-102.55, 23.63],
  moldova: [28.37, 47.41],
  mongolia: [103.85, 46.86],
  morocco: [-7.09, 31.79],
  mozambique: [35.53, -18.67],
  myanmar: [95.96, 21.91],
  nepal: [84.12, 28.39],
  netherlands: [5.29, 52.13],
  "new zealand": [174.89, -40.9],
  nicaragua: [-85.21, 12.87],
  nigeria: [8.68, 9.08],
  "north macedonia": [21.75, 41.61],
  norway: [8.47, 60.47],
  oman: [55.92, 21.51],
  pakistan: [69.35, 30.38],
  panama: [-80.78, 8.54],
  paraguay: [-58.44, -23.44],
  peru: [-75.02, -9.19],
  philippines: [121.77, 12.88],
  poland: [19.15, 51.92],
  portugal: [-8.22, 39.4],
  qatar: [51.18, 25.35],
  romania: [24.97, 45.94],
  russia: [105.32, 61.52],
  "saudi arabia": [45.08, 23.89],
  serbia: [21.0, 44.02],
  singapore: [103.82, 1.35],
  slovakia: [19.7, 48.67],
  slovenia: [14.99, 46.15],
  "south africa": [22.94, -30.56],
  "south korea": [127.77, 35.91],
  korea: [127.77, 35.91],
  spain: [-3.75, 40.46],
  "sri lanka": [80.77, 7.87],
  sweden: [18.64, 60.13],
  switzerland: [8.23, 46.82],
  taiwan: [120.96, 23.7],
  tanzania: [34.89, -6.37],
  thailand: [100.99, 15.87],
  tunisia: [9.54, 33.89],
  turkey: [35.24, 38.96],
  "turkiye": [35.24, 38.96],
  uganda: [32.29, 1.37],
  ukraine: [31.17, 48.38],
  "united arab emirates": [53.85, 23.42],
  "united kingdom": [-3.44, 55.38],
  "united states": [-95.71, 37.09],
  "united states of america": [-95.71, 37.09],
  uruguay: [-55.77, -32.52],
  uzbekistan: [64.59, 41.38],
  venezuela: [-66.59, 6.42],
  vietnam: [108.28, 14.06],
  yemen: [48.52, 15.55],
  zambia: [27.85, -13.13],
  zimbabwe: [29.15, -19.02],
};

/** Common aliases / abbreviations → canonical key in COUNTRY_CENTROIDS. */
const COUNTRY_ALIASES: Record<string, string> = {
  usa: "united states",
  us: "united states",
  "u.s.a.": "united states",
  "u.s.": "united states",
  america: "united states",
  uk: "united kingdom",
  "u.k.": "united kingdom",
  "great britain": "united kingdom",
  britain: "united kingdom",
  england: "united kingdom",
  uae: "united arab emirates",
  "south korea (rok)": "south korea",
  "republic of korea": "south korea",
  holland: "netherlands",
  "the netherlands": "netherlands",
  deutschland: "germany",
  schweiz: "switzerland",
  suisse: "switzerland",
  svizzera: "switzerland",
};

function normalize(name: string): string {
  return name.trim().toLowerCase().replace(/\s+/g, " ");
}

/**
 * Resolve a connection's map coordinates as [longitude, latitude], or null when
 * it has neither explicit coordinates nor a recognizable country.
 */
export function resolveCoords(entry: KubeconfigEntry): [number, number] | null {
  if (
    entry.latitude != null &&
    entry.longitude != null &&
    Number.isFinite(entry.latitude) &&
    Number.isFinite(entry.longitude)
  ) {
    return [entry.longitude, entry.latitude];
  }
  if (entry.country) {
    const key = normalize(entry.country);
    const canonical = COUNTRY_ALIASES[key] ?? key;
    const hit = COUNTRY_CENTROIDS[canonical];
    if (hit) return hit;
  }
  return null;
}

/** A short human label for a connection's location ("Visp, Switzerland"). */
export function locationLabel(entry: KubeconfigEntry): string | null {
  const parts = [entry.city, entry.country].filter((p): p is string => !!p && p.trim() !== "");
  return parts.length ? parts.join(", ") : null;
}
